...
async def run():
    ...
    for index, row in df.iterrows():
        ...
        """,
            row["уникальный номер"],
            row["направление 1"],
            row["направление 2"],
            row["направление 3"],
            row["адрес площадки"],
            str(row["округ площадки"]),
            row["район площадки"],
            str(row["расписание в активных периодах"]),
            str(row["расписание в закрытых периодах"]),
            str(row["расписание в плановом периоде"]),
        )
    await conn.close()
